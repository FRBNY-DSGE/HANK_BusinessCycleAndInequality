...
function SGU(XSS::Array,A::Array,B::Array, m_par::ModelParameters, n_par::NumericalParameters,
    ...
    ############################################################################
    # The A matrix is 30% roughly zero and B matrix is 90% roughly zero => Sparse Jacobian methods may help
    ...
    for i in indexes.Vm # This may not be true in general, in which case sparse Jacobians may really help
        B[i, i] = 1.0
    end
...
function state_loop!(B, Deriv, indexes,n_FD)
    Threads.@threads for i = 1:n_FD:(indexes.Vm[1]-1) #in eachcol(B[:,1:(indexes.Vm[1]-1)])
        aux = Deriv.(i) # Differentiate FSYS at
